...
def generate_report_figures(
    ...
    输出：
      ...
      {name}_02_baseline_mask.png
      {name}_03_light_corrected_mask.png
      {name}_04_candidates.png
      {name}_05_warped.png
      {name}_06_detection.png
    """
    if output_base_dir is None:
        output_base_dir = config.COMPARISON_OUTPUT_DIR / image_name
    ...
    if baseline_result is not None:
        ...
        if b_green is not None:
            save_debug_figure(b_green,
                              output_base_dir / f"{image_name}_02_baseline_mask.png",
                              "Baseline — PCB Mask", cmap="gray")
        # 04 — Candidates
        ...
        if b_board is not None and b_board.success:
            ...
            if candidates_vis is not None and "candidates" in b_board.data:
                ...
                save_debug_figure(vis,
                                  output_base_dir / f"{image_name}_04a_baseline_candidates.png",
                                  "Baseline — Board Corners")
            # 05 — Warped
            warped = b_board.data["warped_image"]
            save_debug_figure(warped,
                              output_base_dir / f"{image_name}_05a_baseline_warped.png",
                              "Baseline — Warped")
            # 06 — Detection
            ...
            save_debug_figure(vis,
                              output_base_dir / f"{image_name}_06a_baseline_detection.png",
                              "Baseline — Detection & Measurement")
    ...
    if light_corrected_result is not None:
        ...
        if l_green is not None:
            save_debug_figure(l_green,
                              output_base_dir / f"{image_name}_03_light_corrected_mask.png",
                              "Light Corrected — PCB Mask", cmap="gray")
        # Highlight mask
        ...
        if hl_mask is not None:
            save_debug_figure(hl_mask,
                              ...
                              "Light Corrected — Highlight Mask", cmap="gray")
        ...
        if l_board is not None and l_board.success:
            ...
            if candidates_vis is not None and "candidates" in l_board.data:
                ...
                save_debug_figure(vis,
                                  output_base_dir / f"{image_name}_04b_light_corrected_candidates.png",
                                  "Light Corrected — Board Corners")
            warped = l_board.data["warped_image"]
            save_debug_figure(warped,
                              output_base_dir / f"{image_name}_05b_light_corrected_warped.png",
                              "Light Corrected — Warped")
            ...
            save_debug_figure(vis,
                              output_base_dir / f"{image_name}_06b_light_corrected_detection.png",
                              "Light Corrected — Detection & Measurement")
